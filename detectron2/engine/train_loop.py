...
class SimpleTrainer(TrainerBase):
    ...
    def run_step(self):
        ...
        for task_name, val in data.items():
            ...
            losses=losses/self.gradAddCount
            losses.backward()
        ...
        """
        self.count +=1
        if  self.count%self.gradAddCount==0:
            
            self.optimizer.step()
            self.optimizer.clear_grad()
            print("optimizer working every %s iters",self.gradAddCount)
        ...
        with paddle.amp.auto_cast():
            ...
            with self.model.no_sync():
                for task_name, val in data.items():
                    ...
                    losses = sum(task_loss_dict.values())       
                    scaled = self.grad_scaler.scale(losses)
                    scaled=scaled/self.gradAddCount   #loss减小  配合梯度累加
                    scaled.backward()
                    loss_dict.update(task_loss_dict)
            fused_allreduce_gradients(list(self.model.parameters()), None)
